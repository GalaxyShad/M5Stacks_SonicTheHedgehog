...
bool Entity::isInCamera(Camera& cam)
{
    ...
    if ( (startPos.x >= cam.getPos().x - DEACT_DELTA) && (startPos.x <= cam.getPos().x + SCREEN_WIDTH + DEACT_DELTA) &&
         (startPos.y >= cam.getPos().y - DEACT_DELTA) && (startPos.y <= cam.getPos().y + SCREEN_HEIGHT + DEACT_DELTA))
        return true;

    if ( (rectR < cam.getPos().x - DEACT_DELTA) || (rectL > cam.getPos().x + SCREEN_WIDTH + DEACT_DELTA) ||
         (rectB < cam.getPos().y - DEACT_DELTA) || (rectT > cam.getPos().y + SCREEN_HEIGHT + DEACT_DELTA))
        return false;
    else
        return true;
}
