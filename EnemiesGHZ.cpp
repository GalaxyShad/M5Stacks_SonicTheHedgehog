#include "include\entities\EnemiesGHZ.h"
...
void Bullet::draw(Camera& cam) {
  ...
  if (dir == 1)
    flip = true;
  anim.draw(cam, pos, flip, false, 0);
}
  ...
  for(int i = 0; i < 30; i++) {
    ...
    if (((trn.getTileVerHeight(lSensorPos) != 0) && 
      ...
      (trn.getTileType(rSensorPos) != TILE_EMPTY))) {
        ...
        int tilePosY = int((pos.y + i) / 16) * 16;
        int height = max(trn.getTileVerHeight(lSensorPos), trn.getTileVerHeight(rSensorPos));
...
void EnBuzz::create()
{
    ...
    type = TYPE_ENEMY;
		anim.create(TEX_OBJECTS, (Frame*)framesObjects);
    animWings.create(TEX_OBJECTS, (Frame*)framesObjects);
...
void EnBuzz::reactingToOthers(std::list<Entity*>& entities)
{
  ...
  if (fired && idleTimer == 24) {
    if (!faceRight) entities.push_back(new Bullet(Vector2f(pos.x - 17, pos.y + 23), 0, -1));
    else entities.push_back(new Bullet(Vector2f(pos.x + 17, pos.y + 23), 0, 1));
  }
}
...
void EnBuzz::draw(Camera& cam)
{
  ...
  animWings.draw(cam, wPos, faceRight, false, 0);
  anim.draw(cam, pos, faceRight, false, 0);
}
